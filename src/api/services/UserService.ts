...
@Service()
export default class UserService extends CRUD<Users> {
  constructor(
  ...
  async register(userInputDTO: IUserInputDTO): Promise<IUserResponseDTO> {
    ...
    const userRecord: Users = await this.userRepo.save(newUser);
  ...
  generateToken(userRecord: Users): string {
